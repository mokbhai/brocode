...
// Purpose: Resolves and seeds Electron userData profile paths during the BroCode rename.
...
const DEV_USER_DATA_DIR_NAME = "brocode-dev";
const PROD_USER_DATA_DIR_NAME = "brocode";
const DEV_LEGACY_USER_DATA_DIR_NAMES = ["t3code-dev", "BroCode (Dev)"] as const;
const PROD_LEGACY_USER_DATA_DIR_NAMES = ["t3code", "BroCode (Alpha)"] as const;
...
export function seedDesktopUserDataProfileFromLegacy(input: {
  ...
  try {
    ...
    FS.writeFileSync(
      Path.join(input.targetPath, "brocode-profile-seed.json"),
      `${JSON.stringify(
        {
          sourcePath,
